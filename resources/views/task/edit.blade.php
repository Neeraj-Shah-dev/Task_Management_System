...
<body>
    ...
    <form action="{{ route('task.update', ['task' => $task]) }}" method="post"
        ...
        <div class="form-group mt-3">
            ...
            @enderror
            <input type="text" name="task_id" id="task_id" class="form-control" value="{{ $task->task_id }}">
        </div>
        ...
        <div class="form-group mt-3">
            ...
            <select name="assigned_to" id="assigned_to" class="form-select">
                @foreach($employees as $employee)
                    <option value="{{ $employee->employee_id }}" {{ $employee->employee_id == $task->assigned_to ? 'selected' : '' }}>{{ $employee->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group mt-5">
            <input type="submit" name="submit" id="submit" class="btn btn-primary" value="Update Task">
        </div>
    </form>
